...
## [0.5.3-beta.5] - 2026-09-11

### Fixed
- 修复长上下文（如 80 万 token）下打开任务会话一直转圈、无法加载的问题。
- 会话加载超时或失败时给出明确提示与重试按钮，不再无限转圈。
- 提高 RPC 帧消息大小上限，避免超大快照被静默丢弃导致会话卡住。
- 会话帧解析异常时自动重新同步，不再让界面停在加载状态。
- 任务详情快照请求延长超时时间，适配长上下文。
...
### Fixed
- 修复多任务运行时列表按时间频繁重排导致的闪屏问题。
- 修复英文界面下部分文案仍为中文（设置页等）。
- 设置页选项归类到「通用 / 调试与高级 / 设备」分区，减少平铺。
- 修复计划模式批准后计划内容显示不正确的问题（保留完整计划与选项答案）。
